...
{{
    settings.stream() << "<div class=\'type\'>";
    @if('{etyName}' != 'uint8_t')
    settings.stream() << object;
    @end if
    @if('{etyName}' == 'uint8_t')
...
{{
    return settings.stream() << "<div class=\'type\'>" << object << "</div>";
}}
@end basetype
...
{{
    ...
    return settings.stream() << object << ")</div>";
}}
@end enum
...
inline std::ostream& dump_html_{flagName}({flagName} object, const ApiDumpSettings& settings, int indents)
{{
    return settings.stream() << "<div class=\'val\'>" << object << "</div>";
}}
@end flag
...
{{
    ...
    @if({memPtrLevel} == 0)
    dump_text_value<const {memBaseType}>(object.{memName}, settings, "{memType}", "{memName}", indents + 1, dump_html_{memTypeID});
    @end if
    @if({memPtrLevel} == 1 and '{memLength}' == 'None')
    dump_text_pointer<const {memBaseType}>(object.{memName}, settings, "{memType}", "{memName}", indents + 1, dump_html_{memTypeID});
    @end if
    @if({memPtrLevel} == 1 and '{memLength}' != 'None' and not {memLengthIsMember})
    dump_text_array<const {memBaseType}>(object.{memName}, {memLength}, settings, "{memType}", "{memChildType}", "{memName}", indents + 1, dump_html_{memTypeID});
    @end if
    @if({memPtrLevel} == 1 and '{memLength}' != 'None' and {memLengthIsMember})
    dump_text_array<const {memBaseType}>(object.{memName}, object.{memLength}, settings, "{memType}", "{memChildType}", "{memName}", indents + 1, dump_html_{memTypeID});
    @end if
...
{{
    ...
    @foreach choice
    @if({chcPtrLevel} == 0)
    dump_text_value<const {chcBaseType}>(object.{chcName}, settings, "{chcType}", "{chcName}", indents + 1, dump_html_{chcTypeID});
    @end if
    @if({chcPtrLevel} == 1 and '{chcLength}' == 'None')
    dump_text_pointer<const {chcBaseType}>(object.{chcName}, settings, "{chcType}", "{chcName}", indents + 1, dump_html_{chcTypeID});
    @end if
    @if({chcPtrLevel} == 1 and '{chcLength}' != 'None')
    dump_text_array<const {chcBaseType}>(object.{chcName}, {chcLength}, settings, "{chcType}", "{chcChildType}", "{chcName}", indents + 1, dump_html_{chcTypeID});
    @end if
    @end choice
    return settings.stream() << "</div>";
}}
...
{{
    const ApiDumpSettings& settings(dump_inst.settings());
    settings.stream() << "Thread " << dump_inst.threadID() << ", Frame " << dump_inst.frameCount() << ":\\n";
    settings.stream() << "<details class=\'fn\'><summary><div class=\'var\'>";
    settings.stream() << "{funcName}({funcNamedParams})</div><div class=\'type\'>{funcReturn}</div>";
    settings.stream() << "<div class=\'val\'>";
    ...
    {{
        @foreach parameter
        @if({prmPtrLevel} == 0)
        dump_text_value<const {prmBaseType}>({prmName}, settings, "{prmType}", "{prmName}", 1, dump_html_{prmTypeID});
        @end if
        @if({prmPtrLevel} == 1 and '{prmLength}' == 'None')
        dump_text_pointer<const {prmBaseType}>({prmName}, settings, "{prmType}", "{prmName}", 1, dump_html_{prmTypeID});
        @end if
        @if({prmPtrLevel} == 1 and '{prmLength}' != 'None')
        dump_text_array<const {prmBaseType}>({prmName}, {prmLength}, settings, "{prmType}", "{prmChildType}", "{prmName}", 1, dump_html_{prmTypeID});
        @end if
        @end parameter
    }}
    settings.shouldFlush() ? settings.stream() << std::endl : settings.stream() << "\\n";
